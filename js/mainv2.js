...
console.log("ios: ", game.device.iOS);
console.log("mobsaf: ", game.device.mobileSafari);
console.log("webapp: ", game.device.webApp);
console.log("iosB: " , game.device.iOSVersion);
*/
/*
var pl1Pos;
var pl2Pos;
...
var turn = 0;
var dieBtn;
//console.log(window.innerWidth * window.devicePixelRatio * 0.75, window.innerHeight * window.devicePixelRatio * 0.75);
var boardParent;
var boardChild;
var dice;
...
var GameState = {
	preload: function(){
		...
		//game.load.image('pl1', 'assets/images/red.png');
		//game.load.image('pl2', 'assets/images/blu.png');
		//game.load.image('pl3', 'assets/images/ylw.png');
		...
		game.load.image('d2', 'assets/images/d2.png');
		game.load.image('d3', 'assets/images/d3.png');
		game.load.image('d4', 'assets/images/d4.png');
		game.load.image('d5', 'assets/images/d5.png');
		game.load.image('d6', 'assets/images/d6.png');
		...
		game.load.image('arw','assets/images/arrow.png');
*/
	...
	create: function(){
		...
		parent.anchor.x = 0.5;
		parent.anchor.y = 0.5;*/
		//parent.x = parent.x - parent.width/2;
		//parent.y = parent.y - parent.height/2;
		//parent.scale.setTo(configuration.scale.ratio, configuration.scale.ratio);
		//initializeSprites();
		...
		switch (players.length) {
			...
			/*
			pl1sprite = parent.addChild(game.add.sprite(pl1Pos[0][0],pl1Pos[0][1], 'pl1anim'));
			//pl1sprite.anchor.x = 0.5;
			pl1sprite.scale.setTo(scaleRate, scaleRate);
			pl1sprite.animations.add('go', [1,2,3,4,5,6,7]);
			pl1sprite.animations.add('come', [8,9,10,11,12,13,14]);
			pl2sprite = parent.addChild(game.add.sprite(pl2Pos[0][0],pl2Pos[0][1], 'pl2anim'));
			pl2sprite.scale.setTo(scaleRate, scaleRate);
			pl2sprite.animations.add('go', [1,2,3,4,5,6,7]);
			pl2sprite.animations.add('come', [8,9,10,11,12,13,14]);
			pl3sprite = parent.addChild(game.add.sprite(pl3Pos[0][0],pl3Pos[0][1], 'pl3anim'));
			pl3sprite.scale.setTo(scaleRate, scaleRate);
			pl3sprite.animations.add('go', [1,2,3,4,5,6,7]);
			pl3sprite.animations.add('come', [8,9,10,11,12,13,14]);
			console.log(pl1Pos[0][0]);

			
			pl1sprite = game.add.sprite(0,0, 'pl1anim');
			pl1sprite.scale.setTo(scaleRate, scaleRate);
			pl1sprite.x = parent.right * .88;
			pl1sprite.y = parent.bottom * .75;
			pl1sprite.animations.add('go', [1,2,3,4,5,6,7]);
			pl1sprite.animations.add('come', [8,9,10,11,12,13,14]);
			pl2sprite = game.add.sprite(pl2Pos[0][0],pl2Pos[0][1], 'pl2anim');
			pl2sprite.animations.add('go', [1,2,3,4,5,6,7]);
		...
		*/
		//parent.addChild(game.make.sprite(-9,-9, 'ylSq'));/*
		/*
		var style = { font: "12px Arial", fill: "#000"};
		for (var l = 10; l <= 1280; l+=40) {
			game.add.text(l, 10, " "+ l+ " ", style);
			for (var li = 10; li <= 720; li+=40) {
				if (li != 10){
				game.add.text(l, li, " "+ li+ " ", style);
			}}
		}
*/
		...
		/*
		exFscrnBtn = game.add.button(fscrnBtn.x, fscrnBtn.y, 'exfscrn', function fsc(){
			exFscrnBtn.visible = false;
			fscrnBtn.visible =  true;
			fullScreener();
		});
		exFscrnBtn.visible = false;
	*/
...
function Play() {
	...
	//console.log('Before: ', players[turn]);
	
	//console.log('TurnFirst:', turn);
	dieRoll();
	//console.log(players[turn].name);
	//console.log("Pos Before", players[turn].position);
	//console.log(dice);
	//removeFromMap(players[turn].position);
	...
	//playAnimTest();
	//addToMap(players[turn].position);
	//console.log(players[turn].position);
	var card = checkCards(dice);//checkCards(players[turn].position);
	//console.log(card);
	//console.log('Play Pos:' ,players[turn]);
	//cardControl(card);
	
	animateBox(card, dice);//players[turn].position);
	turn++;	
	timer++;
...
function loopDelay(delay, cnt, dice) {
	setTimeout(function () {
		...
		if (cnt < 10) {
			...
			sprite.anchor.x = 0.5;
			sprite.anchor.y = 0.5;
			//var sprite = game.add.sprite((game.width/2 - game.cache.getImage(currDie).width/2), game.height/2-game.cache.getImage(currDie).height/2, currDie);
